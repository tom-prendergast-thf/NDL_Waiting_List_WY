...
from pyspark.sql.functions import sequence, explode, to_date, lit, expr, date_sub, add_months, col
...
def group_data_function_HF(df_filtered, intervention_group, control_group, var_hc):
    ...
    df_grouped_by_time_period = (
        df_filtered.groupBy("epp_pid", "time_period", "group", "max_time_covered")
...
def objective2_table(df: DataFrame, investigation_col: str) -> DataFrame:
    ...
    for label in labels:
        df_aggregated = df_aggregated.withColumn(
            ...
            f"percentage_{label}", 
            when(col(f"count_{label}") == "<5", lit("<5")).otherwise(col(f"percentage_{label}"))
        )
...
# TABLE 3 FUNCTION
def objective_3_stats(df, investigation_column, prefix):
    def calculate_stats(filtered_df, time_period, period_label):
        stats_df = filtered_df.groupBy(investigation_column).agg(
            F.count(time_period).alias("pathway_count"),
            F.sum(time_period).alias("sum"),
            F.round(F.mean(time_period), 2).alias("mean"),
            F.expr(f"percentile_approx({time_period}, 0.5)").alias("median"),
            F.round(F.stddev(time_period), 2).alias("stddev"),
            (F.expr(f"percentile_approx({time_period}, 0.75)") - F.expr(f"percentile_approx({time_period}, 0.25)")).alias("iqr"),
            F.sum("ndl_wait_length").alias("ndl_wait_length")
        ).withColumn("time_period", F.lit(period_label))
        
        stats_df = stats_df.withColumn(
            "pathway_weeks",
            F.when(F.col("time_period") == "3m_before", F.col("pathway_count") * 13)
             .when(F.col("time_period") == "3m_after", F.col("pathway_count") * 13)
             .when(F.col("time_period") == "while_waiting", F.round(F.col("ndl_wait_length") / 7, 2))
        )
        
        return stats_df

    time_periods = [
        (f"{prefix}_3m_before", "3m_before"),
        (f"{prefix}_waiting_time", "while_waiting"),
        (f"{prefix}_3m_after", "3m_after")
    ]
    results = []

    for period, label in time_periods:
        filtered_df = df.filter(F.col(period).isNotNull())
        result = calculate_stats(filtered_df, period, label)
        results.append(result)

    final_result = results[0]
    for res in results[1:]:
        final_result = final_result.unionByName(res)

    final_result = final_result.withColumn("utilisation_type", F.lit(prefix))
    final_result = final_result.select(investigation_column, "utilisation_type", "time_period", "pathway_count", "sum", "mean", "median", "stddev", "iqr", "pathway_weeks")
    final_result = final_result.orderBy(investigation_column, F.when(F.col("time_period") == "3m_before", 1)
                                                                .when(F.col("time_period") == "while_waiting", 2)
                                                                .when(F.col("time_period") == "3m_after", 3))
    return final_result
