...
# Ensure the 'group' column is treated as categorical with the correct order
percentage_pd['group'] = pd.Categorical(percentage_pd['group'], categories=group_order, ordered=True)

# Sort the DataFrame based on the categorical order
percentage_pd = percentage_pd.sort_values(by='group')
...
plt.title('Percentage of RTT in Each Group')
...
plt.ylabel('Percentage')
plt.title('RTT Referral Priority Percentages in Each Group')
...
display(waiting_list_df.filter(col("wt")>356*2))
...
waiting_list_others = waiting_list_df.fillna({'epp_referral_priority': 'Unknown'}).groupBy(
    "epp_pid", "epp_tfc", "epp_rtt_start_date", "epp_referral_priority", "wt", "group", "sus_primary_procedure_code", "wlmds_priority_set_last"
).count()

display(waiting_list_others.groupBy("sus_primary_procedure_code", "epp_referral_priority").count())
...
#directory = "../Files"
#file_path = os.path.join(directory, "Table4_6gynea_all.xlsx")
...
columns = ["ndl_age_band", "ndl_imd_quantile", "ndl_ethnicity", "ndl_ltc", "Sex", "Frailty_level"]
tmp=filter_data_function_HF_all_cohort(gb_data_final)
group_stats = calculate_wait_band_distribution_characteristics(tmp, columns)
display(group_stats)
...
# Set the index to 'value' for easier plotting
...
# Plotting the counts for each group as stacked bar (flipped)
fig, ax = plt.subplots(figsize=(12, 8))
group_stats_pd[['19-30 weeks', '31-42 weeks', '<= 18 weeks', '> 42 weeks']].plot(kind='barh', stacked=True, ax=ax)
ax.set_title('Counts of Patients in Different Groups')
ax.set_ylabel('Variable and Value')
ax.set_xlabel('Count')
plt.legend(title='Groups')
plt.tight_layout()
plt.show()

# Plotting the percentages for each group as stacked bar (flipped)
fig, ax = plt.subplots(figsize=(12, 8))
group_stats_pd[['19-30 weeks_percentage', '31-42 weeks_percentage', '<= 18 weeks_percentage', '> 42 weeks_percentage']].plot(kind='barh', stacked=True, ax=ax)
...
group_stats_str = final_df.to_csv(index=False, sep=',', lineterminator='')
display(group_stats_str)
...
# Define the directory and file path
#directory = "../Files"
#file_path = os.path.join(directory, "Table4_3_gynea_all.xlsx")
...
# Define the directory and file path#
#directory = "../Files"
#file_path = os.path.join(directory, "Table4_1_Table_4_2gynea_all.xlsx")
...
# Define the directory and file path
#directory = "../Files"
#file_path = os.path.join(directory, "Table4_4_gynea_all.xlsx")
...
# Define the directory and file path
#directory = "../Files"
#file_path = os.path.join(directory, "Table4_5_gynea_all.xlsx")
